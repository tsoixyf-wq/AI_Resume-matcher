...
解析引擎的单一入口已抽取到 [service.py](backend/app/services/parser/service.py) 的 `ParseResult`/`run_parse_pipeline()`，dev 同步上传和 Celery 异步任务共用同一套逻辑，避免代码重复。

1. **文档加载** ([document_loader.py](backend/app/services/parser/document_loader.py)): PDF (PyMuPDF/pdfplumber) / DOCX (python-docx) / TXT / MD
2. **正则 + spaCy** ([ner_extractor.py](backend/app/services/parser/ner_extractor.py)): 毫秒级提取邮箱/电话/URL/学校/公司
3. **GLiNER** (可选): 零样本实体识别，默认关闭（`ENABLE_GLINER=false`）。LLM 已覆盖同类实体，生产环境建议保持关闭
4. **LLM** ([llm_extractor.py](backend/app/services/parser/llm_extractor.py)): 深度语义理解复杂字段。LLM 失败时自动降级为 NER-only，不阻塞解析流程
5. **技能标准化** ([skill_normalizer.py](backend/app/services/parser/skill_normalizer.py)): 基于 `data/skill_taxonomy.json`（11 大类 200+ 技能）做别名映射和分类
6. **简历分类** ([resume_classifier.py](backend/app/services/parser/resume_classifier.py)): 校招 vs 社招

NER 和 LLM 结果会合并——NER 提取的高置信度字段（email/phone）优先覆盖 LLM 结果，技能做并集去重。解析失败时 DB 记录自动删除，前端的 422 错误会被拦截弹窗提示。
...
- `allow_methods` 已收紧为 `["GET", "POST", "PATCH", "PUT", "DELETE", "OPTIONS"]`
- `allow_headers` 仅允许 `["Content-Type", "X-API-Key", "Authorization"]`
- 添加新 HTTP 方法时需要同步更新 `main.py` 第 57 行

### 硬性条件阈值

`MUST_SKILL_MISSING_THRESHOLD`（默认 0.5）控制硬性条件判定——必备技能缺失超过此比例时触发 `is_hard_pass=true`。在 `.env` 中调整此值可改变筛选严格程度。

### HuggingFace 镜像

国内网络环境可通过 `HF_ENDPOINT=https://hf-mirror.com` 设置镜像站下载 BGE 模型。已在 `config.py` 的 `get_settings()` 中自动注入 `HF_ENDPOINT` 环境变量。
...
│   │   ├── services/
│   │   │   ├── parser/       # 简历解析引擎 (service.py 统一入口 → 加载→NER→LLM→标准化→分类)
...
│   └── package.json
├── data/                     # 样本文件 (6简历+6JD) + skill_taxonomy.json (11 大类 200+ 技能)
...
解析流水线的唯一入口是 `app/services/parser/service.py` 的 `run_parse_pipeline()`。dev 同步上传和 Celery 异步任务都通过它执行解析。新增解析步骤时：
1. 在 `service.py` 的 `run_parse_pipeline()` 中添加新步骤调用
2. `ParseResult` dataclass 中添加新字段
3. Schema (`ParsedResumeData`) 和 ORM（JSONB）自动适配新字段
...
编辑 `data/skill_taxonomy.json`，在对应 category 下添加新技能名称和别名。`SkillNormalizer` 会自动加载该文件进行匹配。
